...
If UserCount > 1 Then
    ...
    listusers = ""
    Title = "détection pirate"
    For Each user In colUsers
        If Not user.Name = "DefaultAccount" And Not user.Name = "Guest" Then
	    listusers = listusers & user.Name & ", "
	End If
    Next
    Msg = "les utilisateurs '" & listusers & "' ont accès à votre PC. Il s'agit peut-être de pirates." & vbCrLf & "voulez vous vérifier leur identité?" 
    ...
    End If
Else
    MsgBox "Pas de compte pirate détecté", vbOKOnly, "détection pirate"
End If
...
If fso.FolderExists(folderPath) Then
    ...
    If folder.Files.Count = 0 And folder.SubFolders.Count = 0 Then
        MsgBox "Pas de programmes de démarrage suspects détectés", vbOKOnly, "détection programmes de démarrage"
    Else
        Msg = "Des programmes suspects au démarrage ont été détectés." & vbCrLf & "voulez vous vérifier leur nature?"
	Title = "détection programmes de démarrage"
	Response = MsgBox(Msg, vbYesNo, Title)
